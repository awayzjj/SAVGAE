...
def format_data(data_name):
    # Load data
    if data_name != 'citeseer' and data_name != 'cora' and data_name != 'pubmed':
    ...
    # 自己加rpr
    adj_ndarray0 = adj_train.copy()
    adj_ndarray1 = adj_ndarray0 - sp.dia_matrix((adj_ndarray0.diagonal()[np.newaxis, :], [0]), shape=adj.shape)
    adj_ndarray2 = adj_ndarray1 + sp.eye(adj_ndarray1.shape[0])
    adj_ndarray = adj_ndarray2.toarray()
    adj_ndarray = adj_ndarray.astype(np.float32)
    for i in range(len(adj_ndarray)):
        adj_ndarray[i] = adj_ndarray[i] / adj_ndarray[i].sum()
    inverse = np.linalg.inv(np.eye(len(adj_ndarray), dtype=np.float32) - 0.5 * adj_ndarray)
    s_rpr = 0.5 * inverse
    ...
    # 自己定义rpr
    adj_label = np.dot(s_rpr, s_rpr.T)
    # adj_label_sp0 = np.where(adj_label > 0.001, adj_label, 0)
    adj_label_sp0 = adj_label

    adj_label_sp = sp.csr_matrix(adj_label_sp0)
    adj_label = sparse_to_tuple(adj_label_sp)
...
def get_optimizer(model_str, model, discriminator, placeholders, pos_weight, norm, num_nodes):
    if model_str == 'arga_ae':
        ...
        d_fake = discriminator.construct(model.embeddings, reuse=True)
        # 自己定义d_real
        d_real = discriminator.construct(model.z_mean, reuse=True)
        # d_real = discriminator.construct(model.hidden1, reuse=True)
